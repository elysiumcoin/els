...
#if defined(NDEBUG)
# error "Elysium cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xef8dd6e1e55c928f37d0c66247142b4e0d30bb440e65154badfcf5e5b3994ffa");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Elysium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Elysium Signed Message:\n";
...
{
    // Elysium: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Elysium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Elysium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1486052409, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Bitcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 60 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04faa60b4bf22c38185cf9c8293216fb3cd798aed5c890d665fbd14fd4ccd5f7b5ed77656d0e1c6632442b9b0863f97171fffaacf7493ac6eb1781ec189cd26df8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1486052409;
        ...
        assert(block.hashMerkleRoot == uint256("0xc11240de56ef88c1c9cf9ef01aa4eb50c9f254340efa22d1121381aa2f9d985a"));
...
unsigned char pchMessageStart[4] = { 0x46, 0x6d, 0xf1, 0xaf }; // Elysium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ElysiumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ElysiumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ElysiumMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running ElysiumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ElysiumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ElysiumMiner, pwallet));
}
